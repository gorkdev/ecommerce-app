  ...
  return (
    <Dialog open={Boolean(user)} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        {/* Keyed by user: switching users remounts the body, so the pending
            role selection resets without any syncing effect. */}
...
function UserDetailBody({ user }: { user: AdminUserListItem }) {
  const { user: me } = useAuth();
  const { data: detail, isPending } = useAdminUser(user.id);
  const updateMutation = useUpdateUserRole();

  const [target, setTarget] = useState<Role | "">("");

  const currentRole: Role = detail?.role ?? user.role;
  const selected: Role = target || currentRole;
  const isSelf = Boolean(me && me.id === user.id);
