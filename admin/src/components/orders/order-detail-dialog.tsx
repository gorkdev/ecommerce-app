...
export function OrderDetailDialog({ order, onOpenChange }: Props) {
  ...
  const [current, setCurrent] = useState<OrderStatus | null>(null);
  ...
  const status = current ?? order?.status ?? "PENDING";
  ...
  const discount = order ? Number(order.discountTotal) : 0;
              ...
              {options.length === 0 ? (
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <select
                    ...
                    value={target}
                    onChange={(e) =>
                      setTarget(e.target.value as OrderStatus | "")
                    }
                  >
